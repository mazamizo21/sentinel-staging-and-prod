    ...
    dataFlows: [
      {
        ...
        destinations: [ 'ws1' ]
        transformKql: 'source | extend tg1=todatetime(detection_ts) | extend tg2=iif(isnull(tg1), todatetime(lastSeen), tg1) | extend tg=iif(isnull(tg2), now(), tg2) | project TimeGenerated=tg, url_s=tostring(url), ip_s=tostring(ip), fileHash_s=tostring(fileHash), domain_s=tostring(domain), protocol_s=tostring(protocol), port_d=toint(port), category_s=tostring(category), risk_d=toint(risk), firstSeen_t=todatetime(firstSeen), lastSeen_t=todatetime(lastSeen), source_s=tostring(source), relationships_s=tostring(relationships), detection_methods_s=tostring(detection_methods), action_s=tostring(action), type_s=tostring(type), identifier_s=tostring(identifier), detection_ts_t=todatetime(detection_ts), object_type_s=tostring(object_type)'
