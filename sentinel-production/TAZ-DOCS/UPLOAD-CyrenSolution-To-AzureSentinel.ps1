#pwsh -NoLogo -ExecutionPolicy Bypass -File ./UPLOAD-CyrenSolution-To-AzureSentinel.ps1

[CmdletBinding()]
param(
    [string]$SolutionName = "CyrenThreatIntelligence",
    [string]$BranchName = "feature/tacitred-ccf-hub-v2threatintelligence",
    [string]$RemoteName = "fork",
    [string]$CommitMessage = "fix: sync CyrenThreatIntelligence package from staging"
)
...
$stagingSolutionPath = Join-Path $repoRoot "Solutions/$SolutionName"
...
try {
    Write-Host "=== Uploading $SolutionName solution to Azure-Sentinel repo ==="
    ...
    if ([string]::IsNullOrWhiteSpace($solutionStatus)) {
        Write-Host "No changes detected in Solutions/$SolutionName. Nothing to commit or push."
        return
    }
