      ...
      items: [
        ...
        // NEW: Threat Intelligence Health Score
        {
          ...
          content: {
            version: 'KqlItem/1.0'
            query: '''
let CurrentData = union Cyren_Indicators_CL, Cyren_Indicators_CL

| summarize CurrentCount = count(), LatestTime = max(TimeGenerated);
let HistoricalAvg = union Cyren_Indicators_CL, Cyren_Indicators_CL
...
'''
        ...
        // NEW: Predictive Threat Forecast (Next 24 Hours)
        {
          ...
          content: {
            version: 'KqlItem/1.0'
            query: '''
let HistoricalData = union Cyren_Indicators_CL, Cyren_Indicators_CL
...
let CurrentRate = toscalar(HistoricalData  | summarize avg(ThreatCount));
...
| summarize 
    ...
    AvgRisk = round(avg(Risk), 1),
    CriticalCount = countif(Risk >= 80),
    Coverage = round(count() * 100.0 / toscalar(Cyren_Indicators_CL  | count), 1)
...
'''
        ...
        // NEW: Automated Response Recommendations
        {
          ...
          content: {
            version: 'KqlItem/1.0'
            query: '''
let TopThreats = union Cyren_Indicators_CL, Cyren_Indicators_CL
...
'''
        ...
        // NEW: Threat Intelligence Metrics Dashboard
        {
          ...
          content: {
            version: 'KqlItem/1.0'
            query: '''
let Current = union Cyren_Indicators_CL, Cyren_Indicators_CL
