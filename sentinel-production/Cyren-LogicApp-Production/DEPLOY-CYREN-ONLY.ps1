...
$logDir = ".\docs\deployment-logs\cyren-only-$ts"
...
$dceTemplate | Out-File "$env:TEMP\cyren-dce.json" -Encoding UTF8

az deployment group create -g $rg --template-file "$env:TEMP\cyren-dce.json" --parameters loc=$loc name='dce-sentinel-ti' -n "cyren-dce-$ts" -o none
...
# PHASE 7: ANALYTICS (NONE - CYREN INGESTION ONLY)
Write-Host "`n═══ PHASE 7: ANALYTICS ═══" -ForegroundColor Cyan
Write-Host 'No Cyren-only analytics are deployed by this script. Use existing Content Hub or custom rules if needed.' -ForegroundColor Gray
...
Write-Host '  • No analytics deployed (ingestion-only package)' -ForegroundColor Gray
