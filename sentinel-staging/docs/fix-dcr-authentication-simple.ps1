# Simple Fix Script for DCR Authentication Issues
...
foreach ($la in $logicAppDetails) {
    if ($la.PrincipalId) {
        ...
        try {
            ...
            if ($dcrRoleAssignments.Count -eq 0) {
        ...
        if ($dceName) {
            try {
                ...
                if ($dceRoleAssignments.Count -eq 0) {
...
Write-Host "  2. Run this script again if needed" -ForegroundColor Gray
